...
#'   n_seq,
#'   M0 = rep(0, 5),
#'   S0 = diag(c(10^2, 2.5^2, 2.5^2, 2.5^2, 2.5^2), 5),
#'   delta = 0.2,
#'   effective_thres = 0.975,
#'   scale_effective_thres = 0,
...
#'   X <- stats::model.matrix( ~ HCQ * LR + CP, data = D)
#'   colnames(X)[1] <- "SoC"
...
#'     arm_ub[i, ] <- hdival[2, ]
#'
#'     is_par_effective[i, ] <- beta_eff[i, ] > pnorm(stats::qnorm(effective_thres)*sqrt(1 + scale_effective_thres*n_max/n_enrolled))
...
#'
#'     is_trt_effective [i, ] <- trt_eff[i, ] > pnorm(qnorm(effective_thres)*sqrt(1 + scale_effective_thres*n_max/n_enrolled))
...
#' tibble_interim_quantities1 <- function(dat, ...) {
...
#' #'
#' #' @importFrom dplyr %>%
#' tibble_model_quantities1 <- function(dat, ...) {
#'   dplyr::bind_rows(parallel::mclapply(dat, function(i) {
...
#' #'
#' #' @importFrom dplyr %>%
#' tibble_trial_quantities1 <- function(dat, ...) {
...
#' #'
#' #' @importFrom dplyr %>%
#' tibble_result_quantities1 <- function(dat, ...) {
