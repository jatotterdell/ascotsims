...
ascot_trial3 <- function(
  ...
  for(i in 1:n_int) {
    ...
    for(dom in c("a", "b", "c")) {
      ...
      is_trt_sup[i, idx1] <- trt_bes_act[i, idx1] > superior_thres       # Superior (best amongst active in domain)
      is_trt_inf[i, idx1] <- trt_bes_act[i, idx1] < inferior_thres / (nact - 1) # Inferior (not best amongst active in domain)
    ...
    if(rar_control) {
      ...
      if(rar_best) {
        ...
        p_rand_trt[i + 1, grepl("a", colnames(XI))] <- brar_fix(
          rar_quant[grepl("a", colnames(XI))], is_trt_active[i, grepl("a", colnames(XI))], rar_scale(i), min_rar[1])
        p_rand_trt[i + 1, grepl("b", colnames(XI))] <- brar_fix(
          rar_quant[grepl("b", colnames(XI))], is_trt_active[i, grepl("b", colnames(XI))], rar_scale(i), min_rar[2])
        p_rand_trt[i + 1, grepl("c", colnames(XI))] <- brar_fix(
          rar_quant[grepl("c", colnames(XI))], is_trt_active[i, grepl("c", colnames(XI))], rar_scale(i), min_rar[3])
